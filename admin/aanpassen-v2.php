              ...
              <div class="modal-body  p-4 ">
                ...
                <div class="">
                  <?php foreach($schooljaren as $schooljaar): ?>
                    <a class="btn btn-lg btn-primary" href="/admin/aanpassen-v2.php?type=<?= $_GET['type'] ?>&vestiging=<?= $_GET['vestiging'] ?>&schooljaar=<?= $schooljaar ?>"><?= $schooljaar; ?>-<?= (int) $schooljaar + 1; ?></a>
                  <?php endforeach; ?>
                </div>
      ...
      <div class="container-fluid p-md-5">
        ...
        <div id="optie-wrapper" class="pt-3" style="display:block">
          <ul class="nav nav-tabs mb-4 selectSchooljaar">
            ...
            <?php foreach($schooljaren as $schooljaar): ?>
              <li class="nav-item">
                <a class="nav-link <?= !empty($_GET['schooljaar']) && $schooljaar == $_GET['schooljaar'] ? 'active text-success font-weight-bold' : ''; ?>" data-schooljaar="<?= $schooljaar; ?>"><?= $schooljaar; ?>-<?= (int) $schooljaar + 1; ?></a>
              </li>
            <?php endforeach; ?>
          </ul>
      ...
      <div class="form-row plaatsen-row {{#if volzet}}is-volzet{{/if}}" data-key="{{id}}" data-plaatsen="{{plaatsen}}" data-school="<?= $vestiging->naam; ?>" data-opleiding="{{opleiding}} {{graad}} {{leerjaar}}">
        ...
        <div class="col-md-1 mb-3">
          <div class="form-check">
            <input class="form-check-input toggle-volzet" {{#if volzet}}checked{{/if}} type="checkbox" data-key="{{id}}" data-target="#volzetVerklaringModal{{id}}" id="volzetVerklaring{{id}}">
