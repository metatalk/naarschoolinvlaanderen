<?php
  /*
    ...
    *
  */
  require 'tocsv/AbstractFile.php';

  require 'tocsv/File/Json.php';

  use OzdemirBurak\JsonCsv\File\Json;

  if($_POST['json']) {
    //jsonToCsv($_POST['json'],$_POST['file']);
    // JSON to CSV
    $fp = fopen(__DIR__ . '/exports/'.$_POST['file'].'.json', 'w');
    fwrite($fp, $_POST['json']);
    fclose($fp);

    $json = new Json(__DIR__ . '/exports/'.$_POST['file'].'.json');
    $csvString = $json->convert();
    if($json->convertAndSave(__DIR__ . '/exports/'.$_POST['file'].'.csv')) {
      echo $_POST['file'].'.csv';
    }
  ...
  function jsonToCsv ($json, $csvFilePath = false, $boolOutputFile = false) {
    ...
    $csvFilePath = __DIR__ . '/exports/'.$csvFilePath;
    ...
    if ($csvFilePath !== false) {
      ...
      if ($f === false) {
        die("Couldn't create the file '.$csvFilePath.' to store the CSV, or the path is invalid. Make sure you're including the full path, INCLUDING the name of the output file (e.g. '../save/path/csvOutput.csv')");
      }
    }
    else {
      $boolEchoCsv = true;
